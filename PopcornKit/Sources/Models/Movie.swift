...
public struct Movie: Media, Equatable, Identifiable {
    ...
    private init(_ map: Map) throws {
        if map.context is TraktContext {
            ...
            self.largeCoverImage = try? map.value("images.poster"); self.largeCoverImage = self.largeCoverImage?.replacingOccurrences(of: "w500", with: "w780").replacingOccurrences(of: "SX300", with: "SX1000").replacingOccurrences(of: "http://", with: "https://")
            self.largeBackgroundImage = try? map.value("images.fanart"); self.largeBackgroundImage = self.largeBackgroundImage?.replacingOccurrences(of: "w500", with: "original").replacingOccurrences(of: "SX300", with: "SX1920").replacingOccurrences(of: "http://", with: "https://")
